...
interface Props { boundaryPoints: LatLng[]; missionPlan: MissionPlan | null; telemetry: TelemetryData | null; telemetryHistory?: TelemetryData[]; onMapClick: (event: LeafletMouseEvent) => void; center: LatLng }

function MapClickHandler({ onMapClick }: Pick<Props, 'onMapClick'>) { useMapEvents({ click: onMapClick }); return null; }

export default function MapPanel({ boundaryPoints, missionPlan, telemetry, telemetryHistory = [], onMapClick, center }: Props) {
